...
public class DashboardHomeController implements FacadeAware {
    ...
    public void initialize() {
        ...
        colBillCashier.setCellValueFactory(c -> new SimpleStringProperty(c.getValue().getUser() != null ? c.getValue().getUser().getFullName() : "N/A"));
    }
    ...
    // Populates all stat cards, tables, and charts based on user role
    private void loadDashboardData() {
        ...
        if (isEmployee) {
            ...
            double score = !reports.isEmpty() ? reports.get(0).getAchievementPercentage() : 0;
        ...
        List<Bill> recent = displayBills.stream().sorted(Comparator.comparing(Bill::getBillDate).reversed()).limit(5).collect(Collectors.toList());
        recentBillsTable.setItems(FXCollections.observableArrayList(recent));

        if (isEmployee) { loadPerformanceSummary(user, allBills); } else { loadTopProducts(allBills); }
        loadStockAlerts(lowStockItems);
        loadSalesChart(displayBills);
    }

    // Shows the employee's monthly performance metrics in the sidebar
    private void loadPerformanceSummary(User user, List<Bill> allBills) {
        ...
        if (!reports.isEmpty()) {
            ...
            long txCount = allBills.stream().filter(b -> b.getUser() != null && b.getUser().getUserId() == user.getUserId())
                    .filter(b -> b.getBillDate().getMonthValue() == month && b.getBillDate().getYear() == year).count();
            double avgBill = txCount > 0 ? report.getTotalSales() / txCount : 0;
            double peakSale = allBills.stream().filter(b -> b.getUser() != null && b.getUser().getUserId() == user.getUserId())
                    .filter(b -> b.getBillDate().getMonthValue() == month && b.getBillDate().getYear() == year)
                    .mapToDouble(Bill::getTotalAmount).max().orElse(0);

            double bonusAmount = report.getBonusAmount();
            String statusMsg;
            if (report.getTargetAmount() <= 0) {
                statusMsg = "- Status: No sales target assigned for this month.";
                ...
                statusMsg = remaining > 0 ? String.format("- Status: Rs. %,.0f left to earn bonus", remaining) : "- Status: Target hit! Next sale earns bonus!";
            }
    ...
    // Builds a 7-day bar chart of sales history
    private void loadSalesChart(List<Bill> allBills) {
        ...
        LocalDate today = LocalDate.now();
        for (int i = 6; i >= 0; i--) dailySales.put(today.minusDays(i), 0.0);
        for (Bill b : allBills) {
            LocalDate date = b.getBillDate().toLocalDate();
            if (dailySales.containsKey(date)) dailySales.put(date, dailySales.get(date) + b.getTotalAmount());
        }
        dailySales.forEach((date, amount) -> series.getData().add(new XYChart.Data<>(date.format(dayFormatter), amount)));
    ...
    // Ranks and displays the top 5 selling products
    private void loadTopProducts(List<Bill> allBills) {
        topProductsList.getChildren().clear();
        Map<String, Integer> productSales = new HashMap<>();
        for (Bill b : allBills) {
            for (BillItem item : b.getItems()) {
                productSales.put(item.getProduct().getName(), productSales.getOrDefault(item.getProduct().getName(), 0) + item.getQuantity());
            }
        }
        productSales.entrySet().stream().sorted((a, b) -> b.getValue().compareTo(a.getValue())).limit(5).forEach(entry -> {
    ...
    // Renders stock alert items in the alerts sidebar
    private void loadStockAlerts(List<InventoryItem> lowStockItems) {
        ...
        for (InventoryItem item : lowStockItems) {
            ...
            if (item.isOverStock()) {
                ...
                stock.setStyle("-fx-text-fill: #f59e0b; -fx-font-size: 11px;");
            } else {
